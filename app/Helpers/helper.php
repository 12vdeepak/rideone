...
function taxi_get_distance_matrix($pick_lat, $pick_lng, $drop_lat, $drop_lng, $traffic = false) {
...
function calculateRideFares($distanceInUnit, $dropoffTimeInSeconds, $service, $coupon = null, $rewardAmount = 0)
{
    $timePrice = 0;
    $distanceUnit = $service['distance_unit'] ?? 'km';
    $minimumDistance = $service['minimum_distance'] ?? 0;

    // Convert distance if needed
    if ($distanceUnit == 'mile') {
        $distanceInUnit = km_to_mile($distanceInUnit);
    }

    $baseFare = $service['base_fare'] ?? 0;
    $timePrice = ($dropoffTimeInSeconds / 60) * ($service['per_minute_drive'] ?? 0);
    
    // Calculate distance price
    $distanceInUnit = max($distanceInUnit - $minimumDistance, 0);
    $distancePrice = $distanceInUnit * ($service['per_distance'] ?? 0);
    $baseAndDistancePrice = $baseFare + $distancePrice;

    // Calculate total amount
    $totalAmount = $baseAndDistancePrice + $timePrice;
    if ($totalAmount < ($service['minimum_fare'] ?? 0)) {
        $totalAmount = $service['minimum_fare'] ?? 0;
    }

    // Handle commission
    if (($service['commission_type'] ?? '') === 'fixed') {
        $commission = ($service['admin_commission'] ?? 0) + ($service['fleet_commission'] ?? 0);
    ...
    // Calculate discount
    $discountAmount = 0;
    $subtotal = $baseAndDistancePrice + $timePrice;
    if ($coupon) {
        if ($coupon->minimum_amount < $totalAmount) {
    ...
    // Apply reward amount as 50% of total fare
    $rewardBasedDiscount = min($totalAmount * 0.50, $rewardAmount);
    $totalAmount -= $rewardBasedDiscount;

    return [
        'distance'              => round($distanceInUnit, 2),
        'minimum_distance_in_km' => $minimumDistance,
