...
class HomeController extends Controller
{
    ...
    public function rewardActivation(Request $request)
    {
        $auth_user = auth()->user();
        $data['dashboard'] = [
            'pending_activation' => User::where('user_type','rider')->where('rewards','InActive')->count(),
            'total_activation' => User::where('rewards','Active')->count(),
            'total_rider' => User::getUser('rider')->count(),
            'total_ride' => RideRequest::count(),
            'today_activation' => RewardActivation::whereDate('activated_at', Carbon::today())->sum('amount'),
            'monthly_activation' => RewardActivation::whereMonth('activated_at', Carbon::now()->month)->sum('amount'),
        ];

        $recent_activation = RewardActivation::where('activated_at','<=', Carbon::now()->format('Y-m-d H:i:s'))->orderBy('id', 'desc')->take(10)->get();
        return view('dashboards.reward-dashboard',compact('data', 'auth_user','recent_activation'));
    }
    ...
    public function saveRewards(Request $request,$user_id){
        $data = $request->all();

        $wallet =  Reward::firstOrCreate(
            [ 'user_id' => $user_id ]
        );
        ...
        try
        {
            ...
            $data['balance'] = $total_amount;
            $data['datetime'] = date('Y-m-d H:i:s');
            $result = RewardHistory::create($data);           
            DB::commit();
        } catch(\Exception $e) {
            DB::rollBack();
            return $e;
