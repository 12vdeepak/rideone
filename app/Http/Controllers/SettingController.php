...
class SettingController extends Controller
{
    ...
    public function layoutPage(Request $request)
    {
        ...
        switch ($page) {
            ...
            case 'reward-setting':
                $page = 'reward-setting';
                $reward_setting = config('constant.reward');
                
                foreach ($reward_setting as $key => $val) {
                    $reward_setting[$key] = Setting::where('key',$key)->pluck('value')->first();
                }
                
                $data  = view('setting.'.$page, compact('reward_setting', 'page'))->render();
                break;
            case 'ride-setting':
    ...
    public function rewardSettingsUpdate(Request $request)
    {
        if(env('APP_DEMO')){
            return redirect()->route('setting.index', ['page' => 'reward-setting'])->withErrors(__('message.demo_permission_denied'));
        }
        $data = $request->all();
        
        foreach(config('constant.reward') as $key => $val){
            $input = [
                'type'  => 'reward',
                'key'   => $key,
                'value' => $data[$key] ?? null,
        ...
        return redirect()->route('setting.index', ['page' => 'reward-setting'])->withSuccess( __('message.updated'));
    }
