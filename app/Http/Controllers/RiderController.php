...
class RiderController extends Controller
{
    ...
    public function show(RideRequestDataTable $dataTable, $id)
    {
        ...
        $walletHistory =  WalletHistory::where('user_id',$data->id)->get();
        $reward =  RewardHistory::where('user_id',$data->id)->get();
        $rewardTotal =  RewardHistory::where('user_id',$data->id)->sum('amount');

        return $dataTable->with('rider_id',$id)->render('rider.show', compact('rewardTotal','pageTitle', 'data', 'profileImage','walletHistory','reward'));
    }
