        ...
        @foreach($wallet_setting as $key => $value)
            <div class="col-md-6 form-group">
                @if($key == 'preset_topup_amount')
                    {{ Form::label($key, __('message.'.$key).' <span data-toggle="tooltip" data-placement="right" title="'.__('message.preset_topup_amount_info').'"><i class="fas fa-question-circle"></i></span>', ['class' => 'form-control-label'], false) }}
                    {{ Form::text($key, $value ?? null, ['placeholder' => '10|50|100|500', 'class' => 'form-control']) }}
                @elseif($key == 'wallet_activation_charge')
